        ...
        <main>
            <section class="container px-4 mx-auto">
                ...
                <div class="flex items-center justify-between mt-6">
                    @if ($siruppenyedia->onFirstPage())
                        <span
                            class="flex items-center px-5 py-2 text-sm text-gray-400 capitalize bg-gray-100 border rounded-md gap-x-2 cursor-not-allowed dark:bg-gray-800 dark:text-gray-500 dark:border-gray-700">
                    ...
                    @else
                        <a href="{{ $siruppenyedia->previousPageUrl() }}"
                    ...
                    <div class="hidden md:flex items-center gap-x-2">
                        @php
    $currentPage = $siruppenyedia->currentPage();
    $lastPage = $siruppenyedia->lastPage();
    $start = max(1, $currentPage - 2);
    $end = min($lastPage, $currentPage + 2);
@endphp

<div class="hidden md:flex items-center gap-x-2">
    {{-- Tampilkan titik di depan jika perlu --}}
    @if ($start > 1)
        <a href="{{ $siruppenyedia->url(1) }}"
           class="px-3 py-1 text-sm rounded-md text-gray-600 hover:bg-gray-100">1</a>
        @if ($start > 2)
    ...
    @for ($i = $start; $i <= $end; $i++)
        <a href="{{ $siruppenyedia->url($i) }}"
           class="px-3 py-1 text-sm rounded-md
           {{ $siruppenyedia->currentPage() == $i ? 'bg-blue-100 text-blue-600' : 'text-gray-600 hover:bg-gray-100' }}">
            {{ $i }}
        </a>
    @endfor

    {{-- Tampilkan titik di belakang jika perlu --}}
    @if ($end < $lastPage)
        @if ($end < $lastPage - 1)
            <span class="px-2">...</span>
        @endif
        <a href="{{ $siruppenyedia->url($lastPage) }}"
           class="px-3 py-1 text-sm rounded-md text-gray-600 hover:bg-gray-100">{{ $lastPage }}</a>
    @endif
</div>

                    </div>

                    @if ($siruppenyedia->hasMorePages())
                        <a href="{{ $siruppenyedia->nextPageUrl() }}"
                            class="flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800">
                            <span>
                                Next
                            </span>
